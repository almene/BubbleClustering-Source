...
void memberate(int v,double r, int& lines, int& features, Rcpp::NumericMatrix& data, int* member, int rewardType, double max){//find membership
  ...
  if (rewardType==1){
    reward=1.0/((double)Z); //compute the reward 1/N
  }
  if (rewardType==2){
    reward=1.0/((double)Z*(double)Z); //compute the reward 1/N^2
  }
  if (rewardType==3){
    reward=1.0/((double)Z)*((max-r)/max); //compute the reward 1/N modified for distance
  }
  if (rewardType==4){
    reward=1.0/((double)Z*(double)Z)*((max-r)/max); //compute the reward 1/N^2 modified for distance
  }
}
...
// Main R program
RcppExport SEXP bubble(SEXP data_matrix, SEXP samples, SEXP columns, SEXP bubbleLevel, SEXP bubbleSize, SEXP RadiiDist, SEXP RewardType){//main routine
  ...
  Rcpp::NumericVector RadType(RadiiDist);
  int RadiiT=RadType[0]; // extract the radii distribution type

  Rcpp::NumericVector RewType(RewardType);
  int RewardT=RewType[0]; // extract the reward type
  ...
  for(i=0;i<SAM;i++){//loop over samples
   ...
   //generate a random critical radius
    ...
    double trueMax = MAXX+minn;
    memberate(j,radius, lines, features, data, member, RewardT, trueMax);        //find the membership fuction
...
RcppExport SEXP bubbleOrder(SEXP data_matrix, SEXP samples, SEXP columns, SEXP bubbleLevel, bool display_progress){//main routine
  ...
  for(i=0;i<divisions;i++){//loop over samples
    ...
    for (int s=0;s<lines;s++){
      j=i%lines;//generate core vertex
    memberate(j,radius, lines, features, data, member, 1, trueMax);        //find the membership fuction
    Associate(lines, member, A);                //update the associator
    }
    //print progress bar and check for abort
    if (Progress::check_abort() ){ return error1;}
